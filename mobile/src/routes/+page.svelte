...
<script lang="ts">
	...
	// Rebuild the rich UI shape from a stored row. Duration/emotion are matched
	// back to the local option lists so icons and colours survive the round-trip.
	function toEntry(e: EnergyEntry): Entry {
		return {
			activity: e.activity,
			timestamp: e.timestamp,
			duration: durations.find((d) => d.label === e.duration_label) ?? null,
			emotion:
				emotions.find((m) => m.label === e.emotion_label) ??
				(e.emotion_emoji
					? { emoji: e.emotion_emoji, label: e.emotion_label ?? '', color: 'from-gray-400 to-gray-600' }
					: null),
			energy: e.energy
		};
	}

	function todayFromCache(): Entry[] {
		try {
			const cached: Entry[] = JSON.parse(localStorage.getItem('energy-entries') || '[]');
			const now = new Date();
	...
	async function refreshEntries() {
		...
		try {
			entries = filterToday(await loadEnergyEntries()).map(toEntry);
		} catch (err) {
			console.error('Failed to load entries:', err);
		}
	}

	// Paint the cache immediately, then let the backend (source of truth) win.
	// Depends only on the token, so the writes below don't re-trigger it.
	$effect(() => {
		if (auth?.accessToken) {
			entries = todayFromCache();
			refreshEntries();
		} else {
			entries = [];
		}
	...
	async function saveEntry() {
		...
		try {
			...
			// Re-read so the log reflects what the backend actually stored.
